...
* Logic:			All the nodes of the arena is fitted in the form of graph.
					Their direction is also defined.
							...
							|A|----#(15)-------#(12)								|w1|		#(1)              #(21)
												|												|				  |
												|				___								|				  |									# :-	node
											 (11)#------#(14)  |DZ1|________________			|			 ____#(7)___							SZ :-	Sorting zone 
												|			   |					|			|			|			|							
							 _					|			   |		Bridge		|			|			|	____	|
							|B|----#(16)-------#(10)-----------|--------------------|-----------#(0)------#(6)	|IP	|	#(8)-----#(22)
												|			   |					|			|			|			|
												|			   |________________	|			|			|____#(9)___|
												|								|DZ2|	#(4)----#(2)			  |	
												|												|				  |
							 _					|		 __						 __				|				  |
							|C|----#(17)-------#(13)	|w4|					|SZ|	#-------#(3)			  #(23)
...
* Logic:				If a is equal to 1 then pin number 3 of port C is given high logic and continuos beep sound comes
...
void angle_rotate(unsigned int Degrees)
{
	...
	else if(node_number==6 && destinationn_node==21 && *navigation_pointer=='W') //special case of feedback constant
	{
		ReqdShaftCount = (float) Degrees/ 3.4;
	}
	else if(node_number==7 && path_index==2)   //special case of feedback constant
	{
		ReqdShaftCount = (float) Degrees/ 3.4;
	}
	else
...
void left_degrees(unsigned int Degrees)
{
	...
	if(node_number==7 && path_index==2)			//special case
	{
		right();
		_delay_ms(300);
		stop();
...
* Logic:					If path string is loaded the bot will rotate till it will not achieve the direction instructed by the string and than it increases the path_index
							...
							block in container (in DZ2 if bot is on city area else in DZ2).
							
							->Node 6 belongs to initial position of waste blocks, so at this node bot will move its arm accordingly and picks up waste (if IP=3 or IP=2) after that 
							...
							For the condition IP equals to 1, the bot will check whether waste is faced in front of node number 6 or not, if yes then the bot picks up the waste and 
							instructed to deposit to the deposition nodes else it is instructed to traverse node 7 through node 21 so that its orientation error got minimized.
							...
							whether the flag color is matching with the color of the waste which is in robotic arm, if yes then bot will deposit the waste else it will instructed towards
							other deposition nodes.
							
							->Node 7,8 and 9 are the nodes which are also associated with initial position of waste block but they are used to identify the face orientation of low 
							chemical concentration waste block, if at any of these nodes the bot finds the orientation of the waste then it will pick up the waste and instruct
							the bot to move towards the waste deposition nodes to deposit the waste.
...
{
	navigate();
	if(node_number==7 && destinationn_node==22 && path[path_index]!='\0')   //special case for particular node
	{
		distance('f',8.5,'o');
...
int main(void)
{
	...
	{
		line_following();
		if(deposition_zone_A+deposition_zone_B+deposition_zone_C==0)
		{
			buzzer(1);
			_delay_ms(5000);
			buzzer(0);
